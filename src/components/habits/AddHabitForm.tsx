...
interface AddHabitFormProps {
  onAdd: (habit: { name: string; description: string; category: string; target_frequency: number }) => void;
  onClose: () => void;
}

export default function AddHabitForm({ onAdd, onClose }: AddHabitFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) return;
    onAdd({ name: name.trim(), description: description.trim(), category, target_frequency: frequency });
  };
      ...
      >
        ...
        >
          <div className="mb-5 flex items-center justify-between">
            <h3 className="text-lg font-bold text-ofira-text">{t('habitSelect.addHabit')}</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder={t('habitSelect.habitNamePlaceholder')}
                className="w-full rounded-lg border border-ofira-border bg-ofira-surface1 px-3 py-2 text-sm text-ofira-text placeholder:text-ofira-text-secondary/50 focus:border-ofira-violet focus:outline-none"
                maxLength={100}
                autoFocus
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder={t('habitSelect.descriptionPlaceholder')}
                className="w-full rounded-lg border border-ofira-border bg-ofira-surface1 px-3 py-2 text-sm text-ofira-text placeholder:text-ofira-text-secondary/50 focus:border-ofira-violet focus:outline-none"
                rows={2}
                maxLength={200}
              />
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                type="submit"
                disabled={!name.trim()}
                className="flex-1 rounded-lg bg-ofira-violet px-4 py-2.5 text-sm font-medium text-white hover:bg-ofira-violet/90 disabled:opacity-50"
              >
